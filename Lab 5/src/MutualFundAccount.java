...
public class MutualFundAccount {
	...
	public String toString() {
		...
		String formattedPosition = myFormatter.format(this.numShares * this.mf.getPricePerShare());
...
class ValueComparator implements Comparator<MutualFundAccount> {
	...
	@Override
	public int compare(MutualFundAccount account1, MutualFundAccount account2) {
		double account1Value = account1.getNumShares() * account1.getMf().getPricePerShare();
		double account2Value = account2.getNumShares() * account2.getMf().getPricePerShare();
		if(account1Value == account2Value) return 0;
		else if(account1Value < account2Value) return -1;
		else return 1;
	}
}